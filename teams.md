...
砂糖国家队/砂国。  
0金唯一竞速阵容，具有极高的上限，在打桩和成本论环境下几乎横扫低金。  
当然不追求上限时强度也很高。  
...
在高倍率低附着频率的嘉明实装后才有比较正常的队伍。  

## 1.4.1 嘉明+风X+冰Y+冰Z

融化嘉明。  
由于四星冰难以兼顾挂冰的高频率（凯亚罗莎莉亚）和长时间（重云莱依拉），使用时对附着的把控有较高要求，而且很难携带班尼特（EQ的强火连放会直接导致极难逆转的火底）或携带时需要班尼特爆堆充能追求一轮只放一个Q。  
...
上下限比较接近的反应类型，比较看练度，对手部乘区的要求相对较低（取决于具体队伍）。  
